...
object Util {
  ...
  def readEdgeListDF(spark: SparkSession, infile: String, isWeighted: Boolean): EitherEdgeRDD = {
    if (isWeighted) {
      ...
      val rowRDD = df.rdd
      val edgeList: RDD[(Long, (Int, Int))] = rowRDD.map {
        case Row(eid: Long, edge: Seq[Int]) =>
          (eid, (edge(0), edge(1)))
      }
      Right(edgeList)
  ...
  def partitionMainsDF(mains: RDD[MainRow], spark: SparkSession, partitionMap: Map[Int, String]): Unit = {
    val rowRDD = mains.map {
      case ((vid: Int, pid: Int), mirrors: Set[Int], neighs: List[(Int, Int)], partitionInDegree: Int) =>
        Row(vid, pid, mirrors.toList, neighs.flatten { case (a, b) => List(a, b) }, partitionInDegree)
    }
  ...
  def partitionMirrorsDF(mirrors: RDD[MirrorRow], spark: SparkSession, partitionMap: Map[Int, String]): Unit = {
    val rowRDD = mirrors.map {
      case ((vid: Int, pid: Int), mainPid: Int, neighs: List[(Int, Int)], partitionInDegree: Int) =>
        Row(vid, pid, mainPid, neighs.flatten { case (a, b) => List(a, b) }, partitionInDegree)
    }
  ...
  def readMainPartitionDF(path: String, spark: SparkSession) = {
    ...
    rowRDD.map {
      case Row(vid: Int, pidsWithMirrors: mutable.WrappedArray[Int], neighs: mutable.WrappedArray[Int], partitionInDegree: Int) =>
        (vid, pidsWithMirrors.toSet, pairs(neighs.toList), partitionInDegree)
    }
  }

  def readMirrorPartitionDF(path: String, spark: SparkSession) = {
    ...
    rowRDD map {
      case Row(vid: Int, mainPartitionId: Int, neighs: mutable.WrappedArray[Int], partitionInDegree: Int) =>
        (vid, mainPartitionId, pairs(neighs.toList), partitionInDegree)
    }
  }

  /**
   * Use pattern matching to resolve Some and None for the outgoing edges and indegree in a row that represents a
   * mirror vertex
   * Some(outgoing edges) => List(outgoing edges)
   * None(outgoing edges) => List()
   * Some(partitionInDegree) => partitionInDegree
   * None(partitionInDegree) => 0
   *
   * @param row
   * @return MirrorRow
   */
  def cleanupMirrorRow(mirrorRow: ((Int, Int), ((Int, Option[Iterable[(Int, Int)]]), Option[Int]))): MirrorRow = {
   ...
   * @return MainRow
   */
  def cleanupMainRow(mainRow: ((Int, Int), ((Set[_ <: Int], Option[Iterable[(Int, Int)]]), Option[Int]))): MainRow = {
  ...
  /**
   * Read all partitions and join them into a single rdd of weighted edges
   * This global knowledge of the (partitioned) edgelist is needed for main/mirror
   * assignment
   * TODO: partitionDir maybe multiple separate nodes on hdfs
   *
   * @param partitionDir directory that stores all partitions
   * @return
   */
  def readPartitionsAndJoin(sc: SparkContext, partitionDir: String, numPartitions: Int, sep: String): RDD[(Int, (Int, Int, Int))] = {
  ...
  def getDegreesByPartition(rdd: RDD[(Int, (Int, Int, Int))]) = {
    val outNeighbors = rdd
      ...
      .map(t => ((t._1._2, t._1._1), t._2.map(t => t match {
  ...
  /**
   * Given the per-partition out degree and in degree of every node in the graph,
   * assign each node to one partition as a main, and (possibly) multiple partitions as a mirror
   *
   * @param degrees
   */
  def partitionAssignment(degrees: RDD[(Int, (Option[Iterable[Int]], Option[Iterable[Int]]))], outNeighbors: RDD[((Int, Int), Iterable[(Int, Int)])], inDegreesPerPartition: RDD[((Int, Int), Int)]) = {
    ...
    vertexMainAndMirrors.cache() // cache the mains in memory so that their correct location can be propagated to their mirrors
    ...
    val nInEdges = mirrorsWithMainPids.map(t => t._4).reduce(_ + _) + mains.map(t => t._4).reduce(_ + _)
